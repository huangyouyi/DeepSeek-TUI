...
- Linux/LAN Mobile Web opencode-style UI parity: `/web` is the formal chat UI,
  `/debug` is the raw test/diagnostics UI, and tools/approvals render inline in
  the chat timeline while the browser remains a thin client.
...
| LF-M13 | Mobile Web `/web` product and `/debug` split | `/web` is the formal chat UI, `/debug` is the raw test/diagnostics UI, tools/approvals render inline in the chat timeline, product settings configure SSH target/server state, and static deep links work on the Rust server; phone/LAN validation remains manual | partial/manual |
...
│  ├─ [done] M1.0. Entry route contract: `/web` is the formal chat UI, `/debug` is the raw test/diagnostics UI, `/` remains compatible, `/web` and `/debug` static deep links serve the SPA, and `/health`/`/event`/`/api/*` are preserved as server routes
│  ├─ [done] M1.1. Product `/web` Chat shell: product chat is the first-use surface, showing user/assistant text, final answer, tools, and approvals inline in the chat timeline from server-owned state
│  ├─ [done] M1.2. Debug `/debug` preservation: Check SSH, Diagnostics, Advanced Command, raw Timeline, recent Audit, Copy report, smoke-test compatibility, and raw diagnostic views remain available under `/debug`
...
│  ├─ [done] M1.5. Browser thin-client boundary: browser renders server state and sends intents only; the Rust server controls model config, API keys, remote execution, approval policy, audit, and message/tool persistence
...
│  └─ [pending/manual] M1.8. Manual phone/LAN handoff: start the LAN server, open `/web` and `/debug` from a phone browser, configure/check SSH target, run first diagnosis, approve explicit high-risk work if created, and confirm final answer/report/debug fallback; evidence remains Linux/LAN Web unless manual user-run testing proves more in a separate accepted bundle
│
